...
#include PRESENTATION_MAP_PANEL  // "presentation/views/panels/MapPanel.h"로 자동 확장
...
// UI 컴포넌트 구현체 include (ProjectMap 사용)
...
MapPanel::MapPanel(wxWindow* parent) : wxPanel(parent, wxID_ANY) {
    // WebView 초기화 코드 (생략)
    hud_ = new presentation::components::MapOverlayHud(this); // HUD 오버레이 초기화 - wxWidgets 자동 관리
...
void MapPanel::PublishPolylineStyle() {
    ...
    const auto [completed, remaining] = presentation::components::SplitPolylineByProgress(currentPolyline_, routeProgress_);
    const int strokeWidth = presentation::components::WidthForZoom(zoomLevel_, dpiScale_);
